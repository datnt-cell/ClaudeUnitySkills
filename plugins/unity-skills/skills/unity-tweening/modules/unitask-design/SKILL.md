...
- Choice between `UniTask`, raw `Task`, and `IEnumerator` at the architecture layer → load [async](../../../unity-scripting/modules/async/SKILL.md)
- DOTween tween → UniTask adapter (`tween.ToUniTask(TweenCancelBehaviour, token)`) → load [dotween-design](../dotween-design/SKILL.md)
- YooAsset handle → UniTask via `handle.ToUniTask()` extension → load [yooasset-design](../../../unity-addressables-yooasset/modules/yooasset-design/SKILL.md)
- Addressables `AsyncOperationHandle.ToUniTask()` rules → load [addressables-design](../../../unity-addressables-yooasset/modules/addressables-design/SKILL.md)
- Performance review of UniTask-heavy code paths (tracker cost, state machine alloc) → load [performance](../../../unity-architecture-advisory/modules/performance/SKILL.md)
- Asmdef layout for UniTask consumers (`Cysharp.Threading.Tasks.asmdef` reference) → load [asmdef](../../../unity-scripting/modules/asmdef/SKILL.md)
