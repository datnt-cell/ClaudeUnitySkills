...
> This skill is the shared foundation for the whole Unity Skills plugin. The plugin splits Unity's 805 REST skills into domain-grouped skills (unity-scene-authoring, unity-rendering, unity-ui, unity-scripting, unity-networking, ...) — each of those skills assumes you've already read the checklist below. Module and reference docs are English-only. Match requests to modules by meaning in any language, and always reply in the user's language.
...
**`POST /skills/batch`** — up to 50 steps per call (`{"steps":[{"skill","args"}],"continueOnError":false}`; `?mode=dryRun` validates every step): the largest round-trip saving → [batch](../unity-dev-workflow/modules/batch/SKILL.md) (in the unity-dev-workflow skill).
