...
Automate when the task involves **bulk creation, precise numeric values across many objects, programmatic hierarchy layout, or searching/renaming many items** — use the gameobject (unity-scene-authoring plugin) REST module instead.
...
- gameobject (unity-scene-authoring plugin)
