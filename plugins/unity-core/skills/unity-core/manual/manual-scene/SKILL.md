...
Automate when the task involves **querying scene contents, batch loading/unloading, dependency analysis, screenshot capture, or switching scenes programmatically across many scenes** — use the scene (unity-scene-authoring plugin) REST module instead.
...
- scene (unity-scene-authoring plugin)
