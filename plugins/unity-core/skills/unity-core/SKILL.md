...
# Unity Skills — Core Protocol

> This skill is the shared foundation for the whole Unity Skills plugin collection. Unity's 805 REST skills are split across domain-grouped plugins (unity-scene-authoring, unity-rendering, unity-ui, unity-scripting, unity-assets-pipeline, unity-animation-timeline, unity-physics-navigation, unity-addressables-yooasset, unity-tweening, unity-dev-workflow, unity-architecture-advisory, unity-editor-tools, ...) — each of those plugins embeds its own summary of the checklist below so it can run standalone; this skill is the full detail for users who install it too. Module and reference docs are English-only. Match requests to modules by meaning in any language, and always reply in the user's language.
...
**`POST /skills/batch`** — up to 50 steps per call (`{"steps":[{"skill","args"}],"continueOnError":false}`; `?mode=dryRun` validates every step): the largest round-trip saving → batch (unity-dev-workflow plugin).
...
Every skill is named `module_verb`, so the prefix is the routing key. `GET /skills/recommend?intent=...` ranks candidates, or read the index — all 82 modules with mode labels, incl. docs-only ones (`manual-*`, `*-design`, `unity-cli`) with no REST skills → [module index](MODULE_INDEX.md). The module index also tells you which `unity-*` skill in this plugin owns each module (see the plugin's top-level README).
...
Current snapshot: `805` REST skills, `54` categories, `82` module doc directories (`54` REST/module docs + `28` advisory docs), Unity `2022.3+`, default timeout `15 minutes`.

Client: a Python (or any HTTP) client talking to the local REST server described above; this plugin ships docs only, bring your own thin HTTP client or use curl.
