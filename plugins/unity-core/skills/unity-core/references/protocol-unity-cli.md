...
> Unity CLI cold start is opt-in (v2.3+). Most sessions do not need it. This protocol was verified against Unity CLI `1.0.0-beta.9`.
...
4. Only if none of the above show a live Editor for this project: run the read-only preflight (`<cliPath> editors -i --format json`; beta6+ also `doctor --ci --format json` — exit `6` stop, `7` retryable — and `projects verify "<projectPath>" --format json` — exit `6` = project broken, detection only), then cold-start with `<cliPath> open "<projectPath>" --args -unityskills-coldstart` (the marker makes the plugin auto-start the REST server regardless of the Auto-start preference), then `unity_skills.wait_for_health()` until REST is ready.
...
- `UNITY_NO_CLI_INVOKED_TELEMETRY=1` — opt out of the per-run usage ping (beta9+).

For build/test/run exit codes (beta6+: `test` exits `8` for failed tests, `6` = no verdict), signal diagnostics (`SIGSEGV/SIGILL/SIGTRAP/SIGFPE/SIGBUS`), and NDJSON terminal-frame handling, see `skills/unity-cli/SKILL.md`. The binary's own `--help` is always the final authority.
