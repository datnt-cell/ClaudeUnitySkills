...
- Asmdef layout for Addressables consumers → load asmdef (unity-scripting plugin)
- Async orchestration across multiple Addressables operations → load async (unity-scripting plugin)
- Architecture-level decisions (Addressables vs YooAsset, group strategy) → load architecture (unity-architecture-advisory plugin)
