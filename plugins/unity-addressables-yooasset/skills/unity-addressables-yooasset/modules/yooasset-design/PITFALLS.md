...
- Symptom: doesn't exist — YooAsset has no `OnValueChanged` event; users copy this change-notification pattern over from multiplayer/networked-variable APIs
...
| `handle.OnValueChanged += ...` | YooAsset has no `OnValueChanged` hook; this is networked-variable vocabulary from an unrelated system. | `handle.Completed += OnLoaded;` or `await handle.Task;` |
